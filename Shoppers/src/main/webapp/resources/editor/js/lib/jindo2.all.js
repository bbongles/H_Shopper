...
jindo._createEle = function(g, f, a, e) {
	...
	case "tbody":
	case "col":
		d.innerHTML = "<table><" + g + ' class="' + c + '">' + f + "</" + g
				+ "></table>";
		break;
	case "tr":
		d.innerHTML = '<table><tbody><tr class="' + c + '">' + f
				+ "</tr></tbody></table>";
		break;
	default:
		...
		while (typeof t._$superClass != "undefined") {
			...
			for ( var x in t._$superClass.prototype) {
				if (t._$superClass.prototype.hasOwnProperty(x)) {
					...
					if (x != "constructor" && x != "_$superClass"
	...
	var getCSS = function(oEl, sKey) {
		if (oEl.currentStyle) {
			...
			if (window.getComputedStyle) {
				return oDocument_dontShrink.defaultView.getComputedStyle(oEl,
...
jindo.$Agent.prototype.navigator = function() {
	var b = {}, h = -1, c = -1, n = this._navigator.userAgent, m = this._navigator.vendor
			|| "", d = this._dm;
	function g(f, e) {
	...
	b.icab = g("iCab", m);
	b.konqueror = g("KDE", m);
	b.mobile = (g("Mobile", n) || g("Android", n) || g("Nokia", n)
			|| g("webOS", n) || g("Opera Mini", n) || g("BlackBerry", n)
			|| (g("Windows", n) && g("PPC", n)) || g("Smartphone", n) || g(
			"IEMobile", n))
			&& !g("iPad", n);
	b.msafari = (!g("IEMobile", n) && g("Mobile", n))
			|| (g("iPad", n) && g("Safari", n));
...
jindo.$A.prototype.splice = function(b, d) {
	var c = this._array.splice.apply(this._array, Array.prototype.slice
			.apply(arguments));
	return jindo.$A(c)
};
...
jindo.$Ajax = function(c, f) {
	...
	case "flash":
		...
		this._request = new jindo.$Ajax.SWFRequest(jindo.$Fn(this.option, this)
				.bind());
		break;
	case "jsonp":
...
jindo.$Ajax.prototype._onload = (function(a) {
	if (a) {
		return function() {
			var b = this._request.readyState == 4
			...
			if (this._request.readyState == 4) {
				try {
					if (this._request.status != 200
						...
						if (!this._is_abort) {
							c = this._options.onload(jindo.$Ajax
									.Response(this._request))
						}
					}
				} finally {
...
jindo.$Ajax.prototype.request = function(c) {
	...
	if (d.postBody && d.type.toUpperCase() == "XHR"
			&& d.method.toUpperCase() != "GET") {
		if (typeof c == "string") {
			g = c
		} else {
	...
	if (d.type.toUpperCase() == "XHR" && d.method.toUpperCase() == "GET"
			&& /MSIE/.test(window.navigator.userAgent)) {
	...
	if (h.addEventListener && !(l.indexOf("Opera") > -1)
			&& !(l.indexOf("MSIE") > -1)) {
		if (this._loadFunc) {
		...
		.$Class(
				{
					...
					_callback : function(a, b, d) {
						...
						if (this.status == 200) {
							if (typeof b == "string") {
								try {
									this.responseText = this.option("decode") ? decodeURIComponent(b)
											: b;
...
jindo.$Ajax.SWFRequest.write = function(a) {
	...
	jindo.$Ajax.SWFRequest._tmpId = "tmpSwf" + (new Date()).getMilliseconds()
			+ Math.floor(Math.random() * 100000);
	var b = "jindo.$Ajax.SWFRequest.loaded";
	jindo.$Ajax._checkFlashLoad();
	document
			.write('<div style="position:absolute;top:-1000px;left:-1000px"><object tabindex="-1" id="'
...
jindo.$Ajax.FrameRequest = jindo.$Class(
		{
			...
			send : function(d) {
				...
				var a;
				var b = [];
				b.push(this._remote + "/ajax_remote_callback.html?method="
						+ this._method);
				var e = new Array;
...
jindo.$H.prototype.length = function() {
	...
	for ( var a in this._table) {
		if (this._table.hasOwnProperty(a)) {
			if (typeof Object.prototype[a] != "undeifned"
					&& Object.prototype[a] === this._table[a]) {
				continue
			}
			b++
		}
...
jindo.$H.prototype.toQueryString = function() {
	...
	for ( var b in this._table) {
		if (this._table.hasOwnProperty(b)) {
			if (typeof (d = this._table[b]) == "object"
					&& d.constructor == Array) {
				for (i = 0; i < d.length; i++) {
					c[c.length] = encodeURIComponent(b) + "[]="
							+ encodeURIComponent(d[i] + "")
				}
			} else {
				c[c.length] = encodeURIComponent(b) + "="
						+ encodeURIComponent(this._table[b] + "")
			}
		}
	}
	return c.join("&")
...
jindo.$Json.fromXML = function(a) {
	...
	var e = function(n, o) {
		n.replace(/([\w\:\-]+)\s*=\s*(?:"((?:\\"|[^"])*)"|'((?:\\'|[^'])*)')/g,
				function(q, p, s, r) {
					o[p] = l((s ? s.replace(/\\"/g, '"') : undefined)
							|| (r ? r.replace(/\\'/g, "'") : undefined))
				})
	};
	var c = function(p) {
		for ( var n in p) {
...
jindo.$Json.prototype.compare = function(a) {
	return jindo.$Json._oldToString(this._object).toString() == jindo.$Json
			._oldToString(jindo.$Json(a).$value()).toString()
};
...
jindo.$Cookie.prototype.set = function(e, f, d, a, b) {
	...
	if (typeof d == "number") {
		c = ";expires="
				+ (new Date((new Date()).getTime() + d * 1000 * 60 * 60 * 24))
						.toGMTString()
	}
	if (typeof a == "undefined") {
		a = ""
	...
	document.cookie = e + "=" + escape(f) + c + "; path=" + b
			+ (a ? "; domain=" + a : "");
	return this
};
...
jindo.$Event = function(b) {
	...
	if (typeof b.relatedTarget != "undefined") {
		...
		if (b.fromElement && b.toElement) {
			this.relatedElement = b[(this.type == "mouseout") ? "toElement"
					: "fromElement"]
		}
	}
};
...
jindo.$Event.prototype.stop = function(c) {
	...
	var f = (window.event && window.event == this._globalEvent) ? this._globalEvent
			: this._event;
...
jindo.$Element.prototype.opacity = function(d) {
	...
	if (!isNaN(d)) {
		...
		if (typeof f.filters != "undefined") {
			d = Math.ceil(d * 100);
			if (typeof f.filters != "unknown"
					&& typeof f.filters.alpha != "undefined") {
	...
	if (typeof f.filters != "undefined") {
		c = (typeof f.filters.alpha == "undefined") ? (a ? 100 : 0)
				: f.filters.alpha.opacity;
		c = c / 100
	} else {
	...
	if (typeof f == "object") {
		...
		for ( var d in f) {
			if (f.hasOwnProperty(d)) {
				...
				if ((IS_FF || IS_OP)
						&& (d == "backgroundPositionX" || d == "backgroundPositionY")) {
					var b = this.css("backgroundPosition").split(/\s+/);
					s = d == "backgroundPositionX" ? s + " " + b[1] : b[0]
							+ " " + s;
	...
	if (c.currentStyle) {
		a = function(l, h) {
			try {
				...
				throw new Error((l.tagName || "document")
						+ "는 css를 사용 할수 없습니다.")
			}
		}
	} else {
		if (window.getComputedStyle) {
			a = function(l, h) {
				try {
					...
					var f = (l.style[h] || m.defaultView.getComputedStyle(l,
							null).getPropertyValue(
							h.replace(/([A-Z])/g, "-$1").toLowerCase()));
					if (h == "textDecoration") {
						f = f.replace(",", "")
					}
					return f
			...
			a = function(g, f) {
				try {
					...
					throw new Error((g.tagName || "document")
							+ "는 css를 사용 할수 없습니다.")
				}
			}
		}
...
jindo.$Element.prototype._setCss = function(c, b, a) {
	if (("#top#left#right#bottom#").indexOf(b + "#") > 0
			&& (typeof a == "number" || (/\d$/.test(a)))) {
	...
	if (this._element.classList) {
		...
		jindo.$Element.prototype.removeClass = function(g) {
			...
			for (var b = c.length - 1; b >= 0; b--) {
				...
				if (this.hasClass(f)) {
					d.className = (" "
							+ d.className.replace(/\s+$/, "").replace(/^\s+/,
									"") + " ").replace(" " + f + " ", " ")
							.replace(/\s+$/, "").replace(/^\s+/, "")
				}
			}
...
jindo.$Element.prototype.text = function(b) {
	...
	if (c != "undefined" && (c == "string" || c == "number" || c == "boolean")) {
		...
		try {
			if (g != "value") {
				g = (typeof d.textContent != "undefined") ? "textContent"
						: "innerText"
			}
			d[g] = b
		} catch (f) {
	...
	if (a.hasTransition) {
		jindo.$Element.prototype.appear = function(g, h) {
			...
			var f = function() {
				h();
				this.show();
				this.removeEventListener(a.name + "End", arguments.callee,
						false)
			};
			var e = this._element;
			var d = this;
...
jindo.$Element.prototype.evalScripts = function(sHTML) {
	var aJS = [];
	sHTML = sHTML.replace(
			new RegExp("<script(\\s[^>]+)*>(.*?)<\/script>", "gi"), function(
					_1, _2, sPart) {
				aJS.push(sPart);
				return ""
			});
...
jindo.$Element.prototype.fireEvent = function(c, b) {
	function d(h, e) {
		...
		if (e) {
			...
			f.button = (e.left ? 1 : 0) + (e.middle ? 4 : 0)
					+ (e.right ? 2 : 0);
	...
	function a(l, g) {
		...
		if (g) {
			...
			case "MouseEvent":
				...
				f.initMouseEvent(l, true, true, null, g.detail || 0,
						g.screenX || 0, g.screenY || 0, g.clientX || 0,
						g.clientY || 0, g.ctrl, g.alt, g.shift, g.meta,
						g.button, g.relatedElement || null);
				break;
			case "KeyboardEvent":
				if (window.KeyEvent) {
					f = document.createEvent("KeyEvents");
					f.initKeyEvent(l, true, true, window, g.ctrl, g.alt,
							g.shift, g.meta, g.keyCode, g.keyCode)
				} else {
					try {
	...
	jindo.$Element.prototype.fireEvent = (typeof this._element.dispatchEvent != "undefined") ? a
			: d;
...
jindo.$Element.prototype.ellipsis = function(g) {
	...
	var b = this.text();
	var a = b.length;
	var f = parseInt(this.css("paddingTop"), 10)
...
jindo.$Element.insertAdjacentHTML = function(n, m, t, p, q) {
	...
	if (d.insertAdjacentHTML
			&& !(/^<(option|tr|td|th|col)(?:.*?)>/.test(m.replace(
					/^(\s|　)+|(\s|　)+$/g, "").toLowerCase()))) {
...
jindo.$Element.prototype.appendHTML = function(a) {
	return jindo.$Element.insertAdjacentHTML(this, a, "beforeEnd",
			"firstChild", jindo.$Fn(function(b) {
				this.append(b)
			}, this).bind())
};
jindo.$Element.prototype.prependHTML = function(a) {
	return jindo.$Element.insertAdjacentHTML(this, a, "afterBegin",
			"lastChild", jindo.$Fn(function(b) {
				this.prepend(b)
			}, this).bind())
};
jindo.$Element.prototype.beforeHTML = function(a) {
	return jindo.$Element.insertAdjacentHTML(this, a, "beforeBegin",
			"firstChild", jindo.$Fn(function(b) {
				this.before(b)
			}, this).bind())
};
jindo.$Element.prototype.afterHTML = function(a) {
	return jindo.$Element.insertAdjacentHTML(this, a, "afterEnd", "lastChild",
			jindo.$Fn(
					function(b) {
						this._element.parentNode.insertBefore(b,
		...
		if (b && f) {
			...
			if (b) {
				...
				for (var h = 0, g = n.length; h < g; h++) {
					d = n[h];
					jindo.$Element._unEventBind(a, d, a["_delegate_" + d
							+ "_func"]);
...
jindo.$Fn.prototype.attach = function(f, d, h) {
	...
	if ((e instanceof Array)
			|| (jindo.$A && (e instanceof jindo.$A) && (e = e.$value()))) {
	...
	if (typeof e.addEventListener != "undefined"
			&& !jindo.$Fn._resizeEventBugInIE(d)) {
		if (n == "domready") {
			n = "DOMContentLoaded"
		} else {
			if (n == "mousewheel" && c.indexOf("WebKit") < 0
					&& !/Opera/.test(c) && !b) {
				n = "DOMMouseScroll"
			} else {
				if (n == "mouseenter" && !b) {
					...
					if (n == "mouseleave" && !b) {
						...
						if (n == "transitionend" || n == "transitionstart") {
							...
							if (n == "animationstart" || n == "animationend"
...
jindo.$Fn.prototype.detach = function(f, c) {
	...
	if ((d instanceof Array)
			|| (jindo.$A && (d instanceof jindo.$A) && (d = d.$value()))) {
	...
	if (typeof d.removeEventListener != "undefined"
			&& !jindo.$Fn._resizeEventBugInIE(c)) {
		if (n == "domready") {
			n = "DOMContentLoaded"
		} else {
...
function isUnCacheAgent() {
	...
	var b = (!(_ua.indexOf("IEMobile") > -1) && (_ua.indexOf("Mobile") > -1))
			|| (c && (_ua.indexOf("Safari") > -1));
	return b && !c && !a
}
...
(function(b) {
	var a = [ "show", "hide", "toggle", "addClass", "removeClass",
			"toggleClass", "fireEvent", "leave", "empty", "appear",
			"disappear", "className", "width", "height", "text", "html", "css",
			"attr" ];
...
jindo.$S.prototype.trim = function() {
	if ("".trim) {
		...
		jindo.$S.prototype.trim = function() {
			return jindo.$S(this._str.replace(/^(\s|　)+/g, "").replace(
					/(\s|　)+$/g, ""))
		}
	}
...
jindo.$S.prototype.format = function() {
	var b = arguments;
	var a = 0;
	var c = this._str.replace(/%([ 0])?(-)?([1-9][0-9]*)?([bcdsoxX])/g,
			function(m, l, g, f, d) {
				var e = b[a++];
				var h = "", n = "";
				f = f ? +f : 0;
...
jindo.$Document.prototype.renderingMode = function() {
	...
	var a = (b.indexOf("WebKit") > -1 && b.indexOf("Chrome") < 0 && navigator.vendor
			.indexOf("Apple") > -1);
	var c;
	if ("compatMode" in this._doc) {
		c = this._doc.compatMode == "CSS1Compat" ? "Standards" : (d ? "Quirks"
				: "Almost")
	} else {
...
jindo.$Template = function(d) {
	...
	if (typeof d == "undefined") {
		...
		if ((c = document.getElementById(d) || d)
				&& c.tagName
				&& (a = c.tagName.toUpperCase())
				&& (a == "TEXTAREA" || (a == "SCRIPT" && c.getAttribute("type") == "text/template"))) {
...
jindo.$Template.prototype.process = function(data) {
	...
	var tpl = (" " + this._str + " ").replace(/\\{/g, leftBrace).replace(
			/\\}/g, rightBrace).replace(/(?!\\)\}\{/g, "}" + key + "{").split(
			jindo.$Template.splitter), i = tpl.length;
	var map = {
		'"' : '\\"',
		"\\" : "\\\\",
		"\n" : "\\n",
	...
	var cb = [
			function(m) {
				return (m.substring(0, 1) == '"' || m.substring(0, 1) == "'" || m == "null") ? m
						: "d." + m
			}, function(m) {
				return map[m] || m
			}, "", "" ];
					...
					.replace(
							...
							function() {
								...
								if (m[5]) {
									...
									_aStr
											.push("	if( (p# && isNaN(i#=parseInt(x#,10))) || (!p# && !t#.propertyIsEnumerable(x#)) ) continue;");
...
jindo.$Date.makeISO = function(d) {
	...
	return new Date(b[1] || 0, parseInt(b[2] || 0, 10) - 1, b[3] || 0, a, c,
			b[6] || 0, b[7] || 0)
};
jindo.$Date.names = {
	month : [ "January", "Febrary", "March", "April", "May", "June", "July",
			"August", "September", "October", "Novermber", "December" ],
	s_month : [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep",
			"Oct", "Nov", "Dec" ],
...
jindo.$Date.prototype.format = function(c) {
	...
	return (c || "").replace(/[a-z]/ig, function g(d) {
		...
		case "N":
			...
			return f[d];
		case "S":
			return (!!(f.S = [ "st", "nd", "rd" ][e.getDate()])) ? f.S
					: (f.S = "th");
		case "z":
			f.z = Math.floor((e.getTime() - (new Date(e.getFullYear(), 0, 1))
					.getTime())
					/ (3600 * 24 * 1000));
			return f.z;
		case "m":
