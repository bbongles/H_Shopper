/**
 * SmartEditor2 NAVER_Library:SE2.8.2.O4259f59 Copyright NAVER Corp. Licensed
 * under LGPL v2 See license text at
 * http://dev.naver.com/projects/smarteditor/wiki/LICENSE
 */
...
(function() {
	var b = /^\$(LOCAL|BEFORE|ON|AFTER)_/, c = /^\$(BEFORE|ON|AFTER)_MSG_APP_READY$/, a = [], d = {};
	nhn.husky.HuskyCore = jindo
			.$Class({
				name : "HuskyCore",
				aCallerStack : null,
				bMobile : jindo.$Agent().navigator().mobile
						|| jindo.$Agent().navigator().msafari,
				$init : function(e) {
					this.htOptions = e || {};
					a.push(this);
				...
				_exec : function(g, f, e) {
					return (this._exec = this.htOptions.oDebugger ? this._execWithDebugger
							: this._execWithoutDebugger).call(this, g, f, e)
				},
				...
				registerBrowserEvent : function(j, k, f, h, e) {
					h = h || [];
					var g = (e) ? jindo.$Fn(this.delayedExec, this).bind(f, h,
							e) : jindo.$Fn(this.exec, this).bind(f, h);
				...
				_execMsgStep : function(e, f, g) {
					return (this._execMsgStep = this.htOptions.oDebugger ? this._execMsgStepWithDebugger
							: this._execMsgStepWithoutDebugger).call(this, e,
							f, g)
				},
					...
					if (c.test(f)) {
						...
						for (h = 0; h < j; h++) {
							if (!k[h]._husky_bRun) {
								...
								if (typeof k[h].$LOCAL_BEFORE_FIRST == "function"
										&& this
												._execHandler(k[h],
														"$LOCAL_BEFORE_FIRST",
														[ f, g ]) === false) {
				...
				_execHandler : function(g, f, e) {
					return (this._execHandler = this.htOptions.oDebugger ? this._execHandlerWithDebugger
							: this._execHandlerWithoutDebugger).call(this, g,
							f, e)
				},
	...
	nhn.husky.HuskyCore.mixin = function(f, p, g, n) {
		var o = [];
		for (var m = 0, e; (e = a[m]); m++) {
			for (var l = 0, q; (q = e.aPlugins[l]); l++) {
				if (q instanceof f) {
					o.push(q);
		...
		.$Class(
				{
					...
					$init : function(a) {
						...
						this.rxLineBreaker = new RegExp("^("
								+ this.sBlockElement + "|"
								+ this.sBlockContainer + ")$");
						this.rxHasBlock = new RegExp(
								"(?:<(?:"
										+ this.sBlockElement
										+ "|"
										+ this.sBlockContainer
										+ ").*?>|style=[\"']?[^>]*?(?:displays?:s?block)[^>]*?[\"']?)",
								"gi");
						this.setWindow(a)
					},
					select : function() {
					...
					pasteHTML : function(e) {
						...
						var p = l.getLineInfo(), o = p.oStart, f = p.oEnd;
						if (o.oLineBreaker && o.oLineBreaker.nodeName === "P"
								&& l.rxHasBlock.test(e)) {
							l.deleteContents();
							var c = o.oLineBreaker.parentNode, b = o.oLineBreaker.nextSibling;
							...
							g.style.cssText = o.oLineBreaker.style.cssText
									.replace(this._rxTextAlign, "");
					...
					_findParentSingleSpan : function(e) {
						...
						for (var b = 0, a = 0, f, c, d = e.childNodes; (c = d[b]); b++) {
							f = c.nodeValue;
					...
					_findSingleChild : function(f) {
						...
						var b = null;
						for (var c = 0, a = 0, g, d, e = f.childNodes; (d = e[c]); c++) {
							g = d.nodeValue;
					...
					_hasCursorHolderOnly : function(a) {
						...
						if (this._rxCursorHolder.test(a.innerHTML)) {
							...
							return this._hasCursorHolderOnly(this
									._findSingleChild(a))
						}
					}
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
	...
	_assignHTMLElements : function(c) {
		...
		this.aAllUI = jindo.$$("[class*=" + this.sUIClassPrefix + "]",
				this.toolbarArea);
		this.elTextTool = jindo.$$.getSingle("div.husky_seditor_text_tool",
				this.toolbarArea);
		...
		if (jindo.$$.getSingle("DIV.se2_icon_tool") != null) {
			this.elFirstToolbarItem = jindo.$$
					.getSingle("DIV.se2_icon_tool UL.se2_itool1>li>button")
		}
	},
	$LOCAL_BEFORE_FIRST : function(a) {
		var c = jindo.$$(">ul>li[class*=" + this.sUIClassPrefix + "]>button",
				this.elTextTool);
		var b = c.length;
		this.elFirstToolbarItem = this.elFirstToolbarItem || c[0];
		this.elLastToolbarItem = c[b - 1];
		this.oApp.registerBrowserEvent(this.toolbarArea, "keydown",
				"NAVIGATE_TOOLBAR", [])
	},
	$init : function(b, a) {
		this._htOptions = a || {};
	...
	$ON_MSG_APP_READY : function() {
		if (this.oApp.bMobile) {
			this.oApp.registerBrowserEvent(this.toolbarArea, "touchstart",
					"EVENT_TOOLBAR_TOUCHSTART")
		} else {
		...
		this.oApp.exec("REGISTER_HOTKEY",
				[ "esc", "FOCUS_EDITING_AREA", [], a ]);
		if (this._htOptions.aDisabled) {
			this._htOptions._sDisabled = ","
					+ this._htOptions.aDisabled.toString() + ",";
	...
	_enableUI : function(d) {
		if (this._htOptions._sDisabled
				&& this._htOptions._sDisabled.indexOf("," + d + ",") > -1) {
			return
		}
		var b, c;
		this.nUIStatus = 0;
...
nhn.husky.SE_EditingAreaManager = jindo
		.$Class({
			...
			$BEFORE_MSG_APP_READY : function(a) {
				this.oApp.exec("ADD_APP_PROPERTY", [ "version",
			...
			$ON_MSG_APP_READY : function() {
				...
				this.oApp.exec("REGISTER_HOTKEY", [ "esc", "CLOSE_LAYER_POPUP",
						[], document ]);
			...
			$ON_FOCUS : function(a) {
				...
				if (!!this.iframeWindow && this.iframeWindow.document.hasFocus
						&& !this.iframeWindow.document.hasFocus()
						&& this.oActivePlugin.sMode == "WYSIWYG") {
			...
			$ON_MSG_EDITING_AREA_RESIZE_STARTED : function() {
				...
				var b = jindo.$$.getSingle("ul[class^='se2_itool']>li.active",
						this.toolbarArea, {
				...
				if (a
						&& (a.className == "se2_itool2" || a.className == "se2_itool4")) {
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo
		.$Class({
			...
			$init : function(b) {
				...
				var e = this.sIFrameSrc, b = this.iframe, d = jindo.$Fn(
						this.initIframe, this).bind(), c = jindo.$Fn(
						function() {
							this.iframe.src = e
						}, this).bind();
				...
				if (this.oApp.oNavigator.ie) {
					...
					if (this.oApp.oNavigator.bGPadBrowser) {
						this.$ON_EVENT_TOOLBAR_TOUCHSTART = function() {
							this._oIERange = this.oApp.getSelection()
									.cloneRange()
						}
					}
				}
			...
			_getResizeHeight : function() {
				var d = this.getDocument().body, b, f, a = [ "width",
						...
						"wordSpacing" ], e, c;
				if (this.oApp.oNavigator.msafari
						|| (!this.oApp.oNavigator.firefox && !this.oApp.oNavigator.safari)) {
					if (this.oApp.oNavigator.ie
			...
			_setBodyHeight : function() {
				...
				var e = this.getDocument().body, l = jindo.$Element(e), g = parseInt(
						l.css("marginTop"), 10)
						+ parseInt(l.css("marginBottom"), 10), f = this.oApp
						.getEditingAreaHeight(), d = f - g, a = l.height(), c, j;
				...
				var b = jindo.$Agent().navigator(), k = (b.ie && b.nativeVersion === 11), h = (this.nBodyHeight_last === a);
				if (!(k && h)) {
					l.css("height", "0px")
				}
			...
			_onIEBeforeDeactivate : function(a) {
				this.oApp.delayedExec(
						"IE_CHECK_EXCEPTION_FOR_SELECTION_PRESERVATION", null,
						0);
				if (this._oIERange) {
					return
				}
			...
			$ON_CHANGE_EDITING_MODE : function(a, b) {
				if (a === this.sMode) {
					...
					if (c.ie && c.nativeVersion > 8) {
						var d = jindo.$$
								.getSingle("DIV.husky_seditor_editing_area_container").childNodes[0];
					...
					if (b) {
						...
						a.onload = jindo
								.$Fn(
										function() {
											if (this.oApp
													&& this.oApp.getEditingMode
													&& this.oApp
															.getEditingMode() === this.sMode) {
												this.oApp
														.exec("RESET_STYLE_STATUS")
											}
										}, this).bind();
						c.appendChild(a)
					}
			...
			setIR : function(h) {
				var c, b = this.oApp.oNavigator, d = b.ie
						&& document.documentMode < 11, g = d ? "" : "<br>";
				if (this.oApp.applyConverter) {
					c = this.oApp.applyConverter("IR_TO_" + this.sMode, h,
							this.oApp.getWYSIWYGDocument())
				} else {
					c = h
				}
				...
				if (d && this.oApp.getEditingMode() === this.sMode) {
					var f = this.oApp.getWYSIWYGDocument().body
							.getElementsByTagName("P");
					for (var a = 0, e = f.length; a < e; a++) {
						if (f[a].childNodes.length === 1
								&& f[a].innerHTML === "&nbsp;") {
							f[a].innerHTML = ""
						}
					}
...
nhn.husky.SE_EditingArea_HTMLSrc = jindo.$Class({
	...
	setIR : function(c) {
		if (c.toLowerCase() === "<br>" || c.toLowerCase() === "<p>&nbsp;</p>"
				|| c.toLowerCase() === "<p><br></p>"
				|| c.toLowerCase() === "<p></p>") {
			c = ""
		}
...
nhn.husky.SE2M_EditingAreaRuler = jindo
		.$Class({
			...
			_assignHTMLElements : function(a) {
				this.elEditingAreaRuler = jindo.$$.getSingle(
						"DIV.se2_editor_mark", a)
			},
			_adjustWysiwygWidth : function() {
				...
				var e = "{"
						+ this.htConfig[this.nRulerWidth].sStyle.replace(/;/ig,
								",").replace(/\"/ig, "") + "}";
				var a = jindo.$Json(e.replace(/(\w+)\s?:\s?([\w\s]*[^,}])/ig,
						'$1:"$2"'));
				b.css(a.toObject());
			...
			$ON_CHANGE_EDITING_MODE : function(a) {
				...
				if ("WYSIWYG" === a && !!this.bUse
						&& !!this.htConfig[this.nRulerWidth]) {
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo
		.$Class({
			...
			$BEFORE_MSG_APP_READY : function() {
				this.oApp.exec("ADD_APP_PROPERTY", [ "isUseVerticalResizer",
						jindo.$Fn(this.isUseVerticalResizer, this).bind() ])
			},
			$ON_MSG_APP_READY : function() {
				if (this.oApp.bMobile) {
					...
					this.oApp.exec("REGISTER_HOTKEY", [ "shift+esc",
							"FOCUS_RESIZER" ]);
					if (this.isUseVerticalResizer()) {
						this.oResizeGrip.style.display = "block";
						if (!!this.welNoticeLayer
								&& !Number(jindo.$Cookie().get(
										this.sCookieNotice))) {
							this.welNoticeLayer.delegate("click",
									"BUTTON.bt_clse", jindo.$Fn(
											this._closeNotice, this).bind());
			...
			isUseVerticalResizer : function() {
				return (typeof (this.htConversionMode) === "undefined"
						|| typeof (this.htConversionMode.bUseVerticalResizer) === "undefined" || this.htConversionMode.bUseVerticalResizer === true) ? true
						: false
			},
			checkResizeGripPosition : function(b) {
				...
				if (b) {
					if (this.nEditingAreaMinHeight > this.oApp
							.getEditingAreaHeight()
							- c) {
						c = (-1)
								* (this.nEditingAreaMinHeight - this.oApp
			...
			_assignHTMLElements : function(b, a) {
				this.oResizeGrip = jindo.$$.getSingle(
						"BUTTON.husky_seditor_editingArea_verticalResizer", b);
				this.elModeToolbar = jindo.$$.getSingle(
						"DIV.se2_conversion_mode", b);
				this.welNoticeLayer = jindo.$Element(jindo.$$.getSingle(
						"DIV.husky_seditor_resize_notice", b));
				this.welConversionMode = jindo
						.$Element(this.oResizeGrip.parentNode)
			},
			...
			_mousedown : function(a) {
				...
				this.oApp.exec("HIDE_ALL_DIALOG_LAYER");
				this.oApp
						.exec("MSG_EDITING_AREA_RESIZE_STARTED", [
								this.$FnMouseDown, this.$FnMouseMove,
								this.$FnMouseUp ])
			},
			...
			_mouseup : function(a) {
				...
				this.oApp
						.exec("MSG_EDITING_AREA_RESIZE_ENDED", [
								this.$FnMouseDown, this.$FnMouseMove,
								this.$FnMouseUp ])
			},
...
nhn.husky.SE_WYSIWYGEnterKey = jindo
		.$Class({
			...
			onIrToDB : function(b) {
				var a = b, d = /<br(\s[^>]*)?\/?>((?:<\/span>)?<\/p>)/gi, c = /(<p[^>]*>)(?:[\s^>]*)(<\/p>)/gi;
				a = a.replace(d, "&nbsp;$2");
				a = a.replace(c, "$1&nbsp;$2");
				return a
			},
			...
			_wrapBlock : function(l, n) {
				var d = this.oApp.getSelection(), k = d.placeStringBookmark(), p = d
						.getLineInfo(), o = p.oStart, e = p.oEnd, m, f, j;
				if (!o.bParentBreak
						|| d.rxBlockContainer.test(o.oLineBreaker.tagName)) {
					l.stop();
					d.deleteContents();
					if (!!o.oNode.parentNode
							&& o.oNode.parentNode.nodeType !== 11) {
						m = this.oApp.getWYSIWYGDocument().createElement(
								this.sLineBreaker);
						d.moveToBookmark(k);
						...
						d.collapseToEnd();
						f = this.oApp.getWYSIWYGDocument().createElement(
								this.sLineBreaker);
				...
				if (this.htBrowser.firefox) {
					...
					if (this.htBrowser.ie) {
						var g = h.parentNode, b = false, a = false;
						if (!h || !g) {
							d.removeStringBookmark(k);
							return
						}
						d.removeStringBookmark(k);
						b = (g.tagName === "U" || nhn.husky.SE2M_Utils
								.findAncestorByTagName("U", g) !== null);
						a = (g.tagName === "S" || g.tagName === "STRIKE" || (nhn.husky.SE2M_Utils
								.findAncestorByTagName("S", g) !== null && nhn.husky.SE2M_Utils
								.findAncestorByTagName("STRIKE", g) !== null));
						if (b || a) {
			...
			_addSpace : function(b) {
				...
				if (f.length > 0) {
					...
					while (g) {
						...
						if (g.nodeType === 3
								&& (g.nodeValue === "&nbsp;"
										|| g.nodeValue === unescape("%u00A0") || g.nodeValue === "\u200B")) {
							b.removeChild(g)
						}
						g = c
					}
...
nhn.husky.SE2M_EditingModeChanger = jindo
		.$Class({
			...
			_assignHTMLElements : function(a) {
				a = jindo.$(a) || document;
				this.elWYSIWYGButton = jindo.$$.getSingle(
						"BUTTON.se2_to_editor", a);
				this.elHTMLSrcButton = jindo.$$.getSingle("BUTTON.se2_to_html",
						a);
				this.elTEXTButton = jindo.$$.getSingle("BUTTON.se2_to_text", a);
				this.elModeToolbar = jindo.$$.getSingle(
						"DIV.se2_conversion_mode", a);
				...
				this.welTEXTButtonLi = jindo
						.$Element(this.elTEXTButton.parentNode)
			},
			$BEFORE_MSG_APP_READY : function() {
				this.oApp.exec("ADD_APP_PROPERTY", [ "isUseModeChanger",
						jindo.$Fn(this.isUseModeChanger, this).bind() ])
			},
			$ON_MSG_APP_READY : function() {
				if (this.oApp.htOptions.bOnlyTextMode) {
					...
					this.oApp.registerBrowserEvent(this.elWYSIWYGButton,
							"click", "EVENT_CHANGE_EDITING_MODE_CLICKED",
							[ "WYSIWYG" ]);
					this.oApp.registerBrowserEvent(this.elHTMLSrcButton,
							"click", "EVENT_CHANGE_EDITING_MODE_CLICKED",
							[ "HTMLSrc" ]);
					this.oApp.registerBrowserEvent(this.elTEXTButton, "click",
							"EVENT_CHANGE_EDITING_MODE_CLICKED", [ "TEXT",
									false ]);
					this.showModeChanger();
					if (this.isUseModeChanger() === false
							&& this.oApp.isUseVerticalResizer() === false) {
						this.elModeToolbar.style.display = "none"
					}
				}
			},
			...
			isUseModeChanger : function() {
				return (typeof (this.htConversionMode) === "undefined"
						|| typeof (this.htConversionMode.bUseModeChanger) === "undefined" || this.htConversionMode.bUseModeChanger === true) ? true
						: false
			},
...
nhn.husky.SE_PasteHandler = jindo
		.$Class({
			...
			$ON_REGISTER_CONVERTERS : function() {
				if (this.htBrowser.ie) {
					this.oApp.exec("ADD_CONVERTER", [ "IR_TO_DB",
							jindo.$Fn(this.irToDb, this).bind() ])
				}
			},
			irToDb : function(b) {
			...
			$ON_MSG_APP_READY : function() {
				this.elEditingAreaContainer = jindo.$$.getSingle(
						"DIV.husky_seditor_editing_area_container", null, {
							oneTimeOffCache : true
						});
				this.oApp.exec("ADD_APP_PROPERTY", [ "filterPastedContents",
						jindo.$Fn(this._filterPastedContents, this).bind() ]);
				if (this.htBrowser.chrome
						|| (this.htBrowser.safari && this.htBrowser.version >= 6)) {
					this.$ON_EVENT_EDITING_AREA_PASTE = this._handlePaste
				}
			},
			_handlePaste : function(d) {
				var c, e, b, f, a = false;
				c = d.$value().clipboardData.getData("text/html");
				e = document.createElement("DIV");
				e.innerHTML = c;
				b = jindo.$$.getSingle("style", e, {
					oneTimeOffCache : true
				});
				if (b) {
					f = b.innerHTML;
					f = f.replace(/"/g, "'");
					this._sStyleFromClipboard = f
				}
				a = this._hasConversionTarget(c);
				if (a) {
					this._preparePaste();
					setTimeout(
							jindo
											...
											function() {
												...
												if (this.zwspStart) {
													if (typeof (this.zwspStart.nodeValue) == "unknown") {
														if (typeof (this.zwspStart.parentNode) != "unknown"
																&& this.zwspStart.parentNode) {
															this.zwspStart.parentNode
																	.removeChild(this.zwspStart)
														}
													} else {
														if (this.zwspStart.nodeValue) {
															this.zwspStart.nodeValue = this.zwspStart.nodeValue
																	.replace(m,
																			"")
														}
														if (this.zwspStart.nodeValue == ""
												...
												if (this.zwspEnd) {
													if (typeof (this.zwspEnd.nodeValue) == "unknown") {
														if (typeof (this.zwspEnd.parentNode) != "unknown"
																&& this.zwspEnd.parentNode) {
															this.zwspEnd.parentNode
																	.removeChild(this.zwspEnd)
														}
													} else {
														if (this.zwspEnd.nodeValue) {
															this.zwspEnd.nodeValue = this.zwspEnd.nodeValue
																	.replace(h,
																			"")
														}
														if (this.zwspEnd.nodeValue == ""
												...
												try {
													...
													if (typeof (JEagleEyeClient) != "undefined") {
														var j = "http://blog.naver.com/hp_SE_PasteHandler.js/_handlePaste";
														var g = l.lineNumber;
														if (!g) {
															g = 0
														}
												...
												k
														.moveToStringBookmark(this._sBM);
												k.collapseToEnd();
												k.select();
												k
														.removeStringBookmark(this._sBM)
											}, this).bind(), 0)
				}
			},
			_hasConversionTarget : function(b) {
				var a = false, c = new RegExp("<("
						+ this.aConversionTarget.join("|") + ")[^>]*>", "i");
				if (b && c.test(b)) {
					a = true
				}
				return a
			...
			_filterPastedContents : function(V, C) {
				...
				var ak = false, ab = false, m = 0, aa = 0, N = false;
				var s, g, f = [], ad = -1, p, am = "", an = [ "BLOCKQUOTE",
						"DD", "DIV", "DL", "FORM", "H1", "H2", "H3", "H4",
						"H5", "H6", "HR", "OL", "P", "TABLE", "UL", "IFRAME" ], Q = new RegExp(
						"^(" + an.join("|") + ")$", "i"), B = false, ah = 0, L = 0, O = [], y = 0;
				var ai = 0;
				var J = [];
				var a = /^<[^!?\/\s>]+(([\s]{0})|([\s]+[^>]+))>/, j = /^<[\/]?([^\s]+)(([\s]{0})|([\s]+[^>]+))>/, S = /^<\/[A-Za-z]+>/, al = /^<[^>]+\/>/, aq = /^<!--[^<>]+-->/, v = /^<!--[^->]+>/, Y = /^<![^->]+-->/, I = /^[\s]+/, af = /^[^<\n]+/, P = /^<[^<>]+>/, ar = /(mso-[^:]+[\s]*:[\s]*)([^;"]*)([;]?)/gi, ae = /(style[\s]*=[\s]*)(["'])([^"']*)(["'])/i, o = /class[\s]*=[\s]*(?:(?:["']([^"']*)["'])|([^\s>]+))/i, X = /(^<table)/gi, F;
				var q = /&quot;/gi;
				var h = "";
				var e = [];
				var D, T, ag;
				...
				var M = "", c = "", x = "", d = "", ac = "", n = "", aj = false, W = false, b = /([^\w-])(width[\s]*=[\s]*)(["']?)([A-Za-z0-9.]+[%]?)([;]?["']?)/i, E = /([^\w-])(height[\s]*=[\s]*)(["']?)([A-Za-z0-9.]+[%]?)([;]?["']?)/i, ap = /(["';\s])(width[\s]*:[\s]*)([A-Za-z0-9.]+[%]?)([;]?)/i, G = /(["';\s])(height[\s]*:[\s]*)([A-Za-z0-9.]+[%]?)([;]?)/i;
				var ao = /([\s]*)(>)/g;
				var u = /span[\s]*=[\s]*"([\d]+)"/i;
				var t = /colspan[\s]*=[\s]*"([\d]+)"/i;
				var w = /rowspan[\s]*=[\s]*"([\d]+)"/i;
				this._doFilter = jindo
						.$Fn(
								function(ay) {
									this._isPastedMultiParagraph = false;
									this._aGoesPreviousParagraph = [];
									var au = false, az = false, at = 0, ax = 0, aC = false;
									s, g, f = [], ad = -1, p, am = "",
											B = false, ah = 0, L = 0, O = [],
											y = 0;
									ai = 0;
												...
												if (this.htBrowser.chrome
													...
													if (e && e.length > 0) {
														for (var aw = 0, aA = e.length; aw < aA; aw++) {
															...
															ag = new RegExp(T);
															if (ag
																	.test(this._sStyleFromClipboard)) {
																z = this._sStyleFromClipboard
																		.match(ag)[1]
															}
															if (z) {
																if (!!ae
													...
													if (af.test(ay)) {
														...
														if (S.test(ay)) {
															...
															if (P.test(ay)) {
																...
																throw new Error(
																		"Unknown Node : If the content isn't invalid, please let us know.")
															}
														}
													}
												}
			...
			_loadToPasteHelper : function() {
				...
				if (this._isPastedMultiParagraph) {
					...
					f.innerHTML = this._sTarget;
					b = f.childNodes
				}
				if (this._aGoesPreviousParagraph
						&& this._aGoesPreviousParagraph.length > 0) {
					e = this._aGoesPreviousParagraph.join("");
					f = document.createElement("DIV");
					f.innerHTML = e;
					g = f.childNodes;
					for (var d = 0, a = g.length; d < a; d++) {
						this.elPasteHelper.appendChild(g[d].cloneNode(true))
					}
					if (b.length > 0) {
						if (!!b.splice) {
							b.splice(0, 1)
			...
			_loadToBody : function() {
				...
				try {
					...
					h.select();
					var p = h.getNodes();
					var d = this._markMatchedElementIndex(p,
							this.aConversionTarget);
					var o = this.oApp
							.getSelection(this.oApp.getWYSIWYGWindow().parent);
					o.setStartBefore(this.elPasteHelper.firstChild);
					o.setEndAfter(this.elPasteHelper.lastChild);
					o.select();
					var g = o.getNodes();
					var n = this._markMatchedElementIndex(g,
							this.aConversionTarget);
					...
					a.parentNode.insertBefore(this.oSelectionClone
							.cloneNode(true), a);
					throw l
				}
			},
...
nhn.husky.SE2M_ExecCommand = jindo
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				...
				this.oApp.exec("REGISTER_HOTKEY", [ "meta+b", "EXECCOMMAND",
						[ "bold", false, false ] ]);
				this.oApp.exec("REGISTER_HOTKEY", [ "meta+u", "EXECCOMMAND",
						[ "underline", false, false ] ]);
				this.oApp.exec("REGISTER_HOTKEY", [ "meta+i", "EXECCOMMAND",
						[ "italic", false, false ] ]);
				this.oApp.exec("REGISTER_HOTKEY", [ "meta+d", "EXECCOMMAND",
						[ "strikethrough", false, false ] ]);
			...
			$BEFORE_EXECCOMMAND : function(a, j, g, f) {
				...
				for (var c = 0, e = b.getNodes(), h; (h = e[c]); c++) {
				...
				if ((!f || !f.bDontAddUndoHistory) && !this._bOnlyCursorChanged) {
					...
					this.oApp.exec("RECORD_UNDO_BEFORE_ACTION", [ a,
							this.oUndoOption ])
				}
				if (this.oNavigator.ie
						&& this.oApp.getWYSIWYGDocument().selection) {
			...
			_checkBlockQuoteCondition_IE : function() {
				...
				var c = false;
				var a;
				if ((g.ie && (g.nativeVersion >= 9 && g.nativeVersion <= 11) && (g.version >= 9 && g.version <= 11))
						|| (this.oApp.oAgent.os().winxp && g.ie && g.nativeVersion <= 8)) {
					var f = this.oApp.getSelection();
					var e = f.commonAncestorContainer;
					var b = nhn.husky.SE2M_Utils
							.findAncestorByTagNameWithCount("BLOCKQUOTE", e);
					a = b.elNode;
					if (a) {
						var d = nhn.husky.SE2M_Utils
			...
			_removeDummyParagraph_IE : function() {
				if (this._elDummyParagraph && this._elDummyParagraph.parentNode) {
					this._elDummyParagraph.parentNode
							.removeChild(this._elDummyParagraph)
				}
			},
				...
				if (f) {
					...
					default:
						if (e.collapsed && this._rxCmdInline.test(c)) {
							var h = e.placeStringBookmark(), k = e
									.getStringBookmark(h), a = k.previousSibling;
							if (!a || a.nodeValue !== "\u200B") {
			...
			_insertBlankLine : function() {
				...
				if (!this.elP1 && !this.elP2) {
					...
					var b = c.previousSibling, a = c.nextSibling;
			...
			_removeBlockQuote : function() {
				var b = "Apple-style-span", h, f, e = this.oApp.getSelection(), a = e.commonAncestorContainer, g = a, d;
				while (a) {
					if (a.tagName === "LI") {
						d = a;
						a = (a.style.cssText === "display: inline !important; ") ? a.parentNode
								: null
					} else {
	...
	$init : function() {
		...
		if (a.ie && a.version > 8) {
			...
			this.$ON_REGISTER_CONVERTERS = function() {
				var b = /\u2060/g;
				this.oApp.exec("ADD_CONVERTER", [ "WYSIWYG_TO_IR",
						jindo.$Fn(function(c) {
							return c.replace(b, "\uFEFF")
						}, this).bind() ])
			}
			...
			_getStyle : function() {
				...
				for (attributeName in a) {
					...
					if (this.oStyle[attributeName] != a[attributeName]) {
						if ((typeof (document.body.currentStyle) != "object")
...
nhn.husky.SE2M_LineStyler = jindo
		.$Class({
			...
			getLineStyle : function(e) {
				...
				setTimeout(jindo.$Fn(
						function(o, n) {
							...
							if (l.length > 0) {
								var m = nhn.husky.SE2M_Utils
										.findAncestorByTagName("TD", o);
								var p = nhn.husky.SE2M_Utils
										.findAncestorByTagName("TD", n);
			...
			setLineStyle : function(m, o, l, a) {
				...
				function h(t, q, r) {
					if (!t) {
						p = true;
						try {
							t = thisRef.oSelection
									.surroundContentsWithNewNode("P")
						} catch (s) {
							t = thisRef.oSelection
									.surroundContentsWithNewNode("DIV")
						}
					}
			...
			_getSelectedNodes : function(d) {
				...
				if (this.oSelection.collapsed) {
					...
					this.oSelection
							.selectNode(this.oSelection.commonAncestorContainer)
				}
				...
				if (b.length === 0) {
					...
					if (c) {
						...
						var e = this.oSelection._document
								.createTextNode("\u00A0");
...
nhn.husky.SE2M_LineHeightWithLayerUI = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		...
		this.oApp.registerLazyMessage([ "SE2M_TOGGLE_LINEHEIGHT_LAYER" ],
				[ "hp_SE2M_LineHeightWithLayerUI$Lazy.js" ])
	}
});
		...
		.$Class(
				{
					...
					$LOCAL_BEFORE_FIRST : function() {
						...
						if (!this.oApp.bMobile) {
							this.oApp.registerBrowserEvent(
									this.elBackgroundColor, "mouseover",
									"EVENT_MOUSEOVER_COLOR_PALETTE");
							this.oApp.registerBrowserEvent(
									this.elColorPaletteLayer, "mouseover",
									"EVENT_MOUSEOVER_COLOR_PALETTE");
							this.oApp.registerBrowserEvent(
									this.elBackgroundColor, "mouseout",
					...
					$ON_APPLY_COLOR : function(a) {
						...
						if (b.hasClass("husky_se2m_color_palette")) {
							this.oApp
									.exec(
											"COLOR_PALETTE_APPLY_COLOR",
											[
													c,
													nhn.husky.SE2M_Configuration.SE2M_ColorPalette.bAddRecentColorFromDefault ])
						} else {
		...
		.$Class({
			...
			_assignHTMLElements : function(a) {
				this.elLastUsed = jindo.$$.getSingle(
						"SPAN.husky_se2m_fontColor_lastUsed", a);
			...
			$BEFORE_MSG_APP_READY : function() {
				this.oApp.exec("ADD_APP_PROPERTY", [ "getLastUsedFontColor",
						jindo.$Fn(this.getLastUsedFontColor, this).bind() ])
			},
			$ON_MSG_APP_READY : function() {
				...
				this.oApp.registerLazyMessage([ "APPLY_LAST_USED_FONTCOLOR",
						"TOGGLE_FONTCOLOR_LAYER" ],
						[ "hp_SE2M_FontColor$Lazy.js" ])
			},
			...
			getLastUsedFontColor : function() {
				return (!!this.sLastUsedColor) ? this.sLastUsedColor
						: "#000000"
			}
		});
nhn.husky.SE2M_BGColor = jindo.$Class({
	...
	_assignHTMLElements : function(a) {
		this.elLastUsed = jindo.$$.getSingle(
				"SPAN.husky_se2m_BGColor_lastUsed", a);
		this.elDropdownLayer = jindo.$$.getSingle(
	...
	$BEFORE_MSG_APP_READY : function() {
		this.oApp.exec("ADD_APP_PROPERTY", [ "getLastUsedBackgroundColor",
				jindo.$Fn(this.getLastUsedBGColor, this).bind() ])
	},
	$ON_MSG_APP_READY : function() {
		...
		this.oApp.registerBrowserEvent(this.elBGColorList, "click",
				"EVENT_APPLY_BGCOLOR", []);
		this.oApp.registerLazyMessage([ "APPLY_LAST_USED_BGCOLOR",
				"TOGGLE_BGCOLOR_LAYER" ], [ "hp_SE2M_BGColor$Lazy.js" ])
	},
		...
		.$Class({
			...
			$BEFORE_MSG_APP_READY : function() {
				...
				if (a && a.bAutolink === false) {
					...
					try {
						this.oApp.getWYSIWYGDocument().execCommand(
								"AutoUrlDetect", false, false)
					} catch (b) {
					}
				}
			},
			$ON_MSG_APP_READY : function() {
				...
				this.oApp.exec("REGISTER_HOTKEY", [ "ctrl+k",
						"TOGGLE_HYPERLINK_LAYER", [] ]);
				this.oApp.registerLazyMessage([ "TOGGLE_HYPERLINK_LAYER",
						"APPLY_HYPERLINK" ], [ "hp_SE2M_Hyperlink$Lazy.js" ])
			},
			$ON_REGISTER_CONVERTERS : function() {
			...
			$ON_DISABLE_MESSAGE : function(b) {
				...
				try {
					this.oApp.getWYSIWYGDocument().execCommand("AutoUrlDetect",
							false, false)
				} catch (a) {
				}
			...
			irToDb : function(k) {
				...
				for (var r = 0, m = d.length; r < m; r++) {
					...
					try {
						...
						u.innerHTML = (b + u.innerHTML)
								.replace(
										/(&nbsp|\s)?(((?!http[s]?:\/\/)www\.(?:(?!\&nbsp;|\s|"|').)+)|(http[s]?:\/\/(?:(?!&nbsp;|\s|"|').)+))/ig,
		...
		.$Class({
			...
			addAllFonts : function() {
				...
				if (!this.htOptions || !this.htOptions.aDefaultFontList
					...
					this.addFont("돋움체,DotumChe,AppleGothic", "돋움체", 0, "", "",
							1, null, true);
					this.addFont("굴림,Gulim", "굴림", 0, "", "", 1, null, true);
					this.addFont("굴림체,GulimChe", "굴림체", 0, "", "", 1, null,
							true);
					this.addFont("바탕,Batang,AppleMyungjo", "바탕", 0, "", "", 1,
							null, true);
					this.addFont("바탕체,BatangChe", "바탕체", 0, "", "", 1, null,
							true);
					this.addFont("궁서,Gungsuh,GungSeo", "궁서", 0, "", "", 1,
							null, true);
					this.addFont("Arial", "Arial", 0, "", "", 1, "abcd", true);
					this
							.addFont("Tahoma", "Tahoma", 0, "", "", 1, "abcd",
									true);
					this.addFont("Times New Roman", "Times New Roman", 0, "",
							"", 1, "abcd", true);
					this.addFont("Verdana", "Verdana", 0, "", "", 1, "abcd",
							true);
					this.addFont("Courier New", "Courier New", 0, "", "", 1,
							"abcd", true)
				}
				if (!!this.aAdditionalFontList
						&& this.aAdditionalFontList.length > 0) {
					for (c = 0, nLen = this.aAdditionalFontList.length; c < nLen; c++) {
						this.addFont(this.aAdditionalFontList[c][0],
								this.aAdditionalFontList[c][1], 0, "", "", 1)
					}
				}
			},
			$ON_MSG_APP_READY : function() {
			...
			_initFontName : function() {
				...
				this.addAllFonts();
				var a;
				if (this.oApp.getCurrentStyle
						&& (a = this.oApp.getCurrentStyle())) {
			...
			_addFontInMarkup : function() {
				for (var c = 0, b, d, a, e; (b = this.aelFontInMarkup[c]); c++) {
					if (b.firstChild) {
						d = this._getFontFamilyFromLI(b).replace(this._rxQuote,
								"").replace(this._rxComma, ",");
						e |= this._checkFontLI(b, d)
					} else {
						if (b.className.indexOf(this.FONT_SEPARATOR) > -1) {
							if (a) {
			...
			_onMouseupCover : function(f) {
				...
				var g = f.mouse(), c = this.oApp.getWYSIWYGDocument().body, a = jindo
				...
				if (this.oApp.oNavigator.ie && document.documentMode < 11
					...
					if (this.oApp.oNavigator.ie
							&& this.oApp.oNavigator.nativeVersion == 11
			...
			_findLI : function(a) {
				...
				if (a.className.indexOf(this.FONT_SEPARATOR) > -1) {
					return null
				}
				return a
			},
...
nhn.husky.SE2M_Accessibility = jindo
		.$Class({
			...
			_assignHTMLElements : function(a) {
				this.elHelpPopupLayer = jindo.$$.getSingle(
						"DIV.se2_accessibility", a);
				this.welHelpPopupLayer = jindo.$Element(this.elHelpPopupLayer);
				this.oCloseButton = jindo.$$.getSingle("BUTTON.se2_close",
						this.elHelpPopupLayer);
				this.oCloseButton2 = jindo.$$.getSingle("BUTTON.se2_close2",
						this.elHelpPopupLayer);
			...
			$ON_MSG_APP_READY : function() {
				this.htAccessOption = nhn.husky.SE2M_Configuration.SE2M_Accessibility
						|| {};
				this.oApp.exec("REGISTER_HOTKEY", [ "alt+F10",
						"FOCUS_TOOLBAR_AREA", [] ]);
				this.oApp.exec("REGISTER_HOTKEY", [ "alt+COMMA",
						"FOCUS_BEFORE_ELEMENT", [] ]);
				this.oApp.exec("REGISTER_HOTKEY", [ "alt+PERIOD",
						"FOCUS_NEXT_ELEMENT", [] ]);
				if ((this.sType == "basic" || this.sType == "light")
						&& (this.sLang != "ko_KR")) {
					return
				} else {
					this.oApp.exec("REGISTER_HOTKEY", [ "alt+0",
							"OPEN_HELP_POPUP", [] ]);
					this.oApp.exec("REGISTER_HOTKEY", [ "esc",
							"CLOSE_HELP_POPUP", [], document ])
				}
				if (this.htAccessOption.sTitleElementId) {
					this.oApp
							.registerBrowserEvent(
									document
											.getElementById(this.htAccessOption.sTitleElementId),
									"keydown", "MOVE_TO_EDITAREA", [])
				}
			},
			...
			$LOCAL_BEFORE_FIRST : function(a) {
				jindo.$Fn(
						jindo.$Fn(this.oApp.exec, this.oApp).bind(
								"CLOSE_HELP_POPUP", [ this.oCloseButton ]),
						this).attach(this.oCloseButton, "click");
				jindo.$Fn(
						jindo.$Fn(this.oApp.exec, this.oApp).bind(
								"CLOSE_HELP_POPUP", [ this.oCloseButton2 ]),
						this).attach(this.oCloseButton2, "click");
			...
			$ON_FOCUS_NEXT_ELEMENT : function() {
				...
				if (this.htAccessOption.sNextElementId) {
					this._currentNextFocusElement = document
							.getElementById(this.htAccessOption.sNextElementId)
				} else {
					this._currentNextFocusElement = this
							._findNextFocusElement(this.elAppContainer)
				}
				if (this._currentNextFocusElement) {
					window.focus();
					this._currentNextFocusElement.focus()
				} else {
					if (parent && parent.nhn && parent.nhn.husky
							&& parent.nhn.husky.EZCreator
							&& parent.nhn.husky.EZCreator.elIFrame) {
						parent.focus();
						if (this._currentNextFocusElement = this
								._findNextFocusElement(parent.nhn.husky.EZCreator.elIFrame)) {
							this._currentNextFocusElement.focus()
						}
					}
				}
			...
			_switchToSiblingOrNothing : function(a, c) {
				...
				if (c) {
					if (b.previousSibling) {
						...
						while (b.nodeName.toUpperCase() != "BODY"
								&& !b.previousSibling) {
							b = b.parentNode
						}
				...
				if (d) {
					...
					if (f) {
						for (var a = e.childNodes.length, c = a - 1; c >= 0; c--) {
							b._recursivePreorderTraversalFilter(
									e.childNodes[c], g, true);
			...
			_isFocusTag : function() {
				...
				for (var d = 0, a = e.length; d < a; d++) {
					if (c.nodeType === 1 && c.nodeName
							&& c.nodeName.toUpperCase() == e[d] && !c.disabled
							&& jindo.$Element(c).visible()) {
						b = true;
						break
					}
				}
			...
			$ON_FOCUS_BEFORE_ELEMENT : function() {
				...
				if (this.htAccessOption.sBeforeElementId) {
					this._currentPreviousFocusElement = document
							.getElementById(this.htAccessOption.sBeforeElementId)
				} else {
					this._currentPreviousFocusElement = this
							._findPreviousFocusElement(this.elAppContainer)
				}
				if (this._currentPreviousFocusElement) {
					...
					if (parent && parent.nhn && parent.nhn.husky
							...
							&& parent.nhn.husky.EZCreator.elIFrame) {
						parent.focus();
						if (this._currentPreviousFocusElement = this
								._findPreviousFocusElement(parent.nhn.husky.EZCreator.elIFrame)) {
							this._currentPreviousFocusElement.focus()
						}
					}
				}
			...
			$ON_FOCUS_TOOLBAR_AREA : function() {
				this.oButton = jindo.$$.getSingle("BUTTON.se2_font_family",
						this.elAppContainer);
			...
			$ON_OPEN_HELP_POPUP : function() {
				...
				this.nCalcX = this.htTopLeftCorner.x
						+ this.oApp.getEditingAreaWidth() - this.nLayerWidth;
...
nhn.husky.SE2M_SCharacter = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		...
		this.oApp.registerLazyMessage([ "TOGGLE_SCHARACTER_LAYER" ],
				[ "hp_SE2M_SCharacter$Lazy.js" ])
	}
});
nhn.husky.SE2M_FindReplacePlugin = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		...
		this.oApp.registerLazyMessage([ "TOGGLE_FIND_REPLACE_LAYER",
				"SHOW_FIND_LAYER", "SHOW_REPLACE_LAYER",
				"SHOW_FIND_REPLACE_LAYER" ], [
				"hp_SE2M_FindReplacePlugin$Lazy.js", "N_FindReplace.js" ])
	},
...
nhn.husky.SE2M_Quote = jindo
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				...
				this.oApp.registerLazyMessage([ "TOGGLE_BLOCKQUOTE_LAYER" ],
						[ "hp_SE2M_Quote$Lazy.js" ])
			},
...
nhn.husky.SE2M_TableCreator = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		this.sReEditGuideMsg_table = this.oApp
				.$MSG("SE2M_ReEditAction.reEditGuideMsg.table");
		this.oApp.exec("REGISTER_UI_EVENT", [ "table", "click",
				"TOGGLE_TABLE_LAYER" ]);
		this.oApp.registerLazyMessage([ "TOGGLE_TABLE_LAYER" ],
				[ "hp_SE2M_TableCreator$Lazy.js" ])
	},
	$ON_REGISTER_CONVERTERS : function() {
		this.oApp.exec("ADD_CONVERTER_DOM", [ "IR_TO_DB",
...
nhn.husky.SE2M_StyleRemover = jindo
		.$Class({
			name : "SE2M_StyleRemover",
			$ON_MSG_APP_READY : function() {
				this.oApp.exec("REGISTER_UI_EVENT", [ "styleRemover", "click",
						"CHOOSE_REMOVE_STYLE", [] ])
			},
			$LOCAL_BEFORE_FIRST : function() {
			...
			$ON_REMOVE_STYLE : function(e) {
				...
				for (var d = 0; d < b.length; d++) {
					var g = b[d];
					if (g.style && g.tagName != "BR" && g.tagName != "TD"
							&& g.tagName != "TR" && g.tagName != "TBODY"
							&& g.tagName != "TABLE") {
						g.removeAttribute("align");
						g.removeAttribute("style");
						if ((jindo.$Element(g).css("display") == "inline"
								&& g.tagName != "IMG" && g.tagName != "IFRAME")
								&& (!g.firstChild || e
										._isBlankTextNode(g.firstChild))) {
							g.parentNode.removeChild(g)
						}
					}
				}
				...
				var c = this._document
						.getElementById(e.HUSKY_BOOMARK_START_ID_PREFIX + f);
				while (c) {
					oParent = nhn.DOMFix.parentNode(c);
					oParent.removeChild(c);
					while (oParent
							&& (!oParent.firstChild || (!oParent.firstChild.nextSibling && e
									._isBlankTextNode(oParent.firstChild)))) {
				...
				var c = this._document
						.getElementById(e.HUSKY_BOOMARK_END_ID_PREFIX + f);
				while (c) {
					oParent = nhn.DOMFix.parentNode(c);
					oParent.removeChild(c);
					while (oParent
			...
			_removeStyle : function(f) {
				...
				for (var d = 0; d < f.length; d++) {
					...
					if (k.firstChild) {
						if (c
								.indexOf(this.oHuskyRange
										._getVeryLastRealChild(k)) == -1) {
							continue
						}
						if (c.indexOf(this.oHuskyRange
								._getVeryFirstRealChild(k)) == -1) {
							continue
						}
					}
					if (!k.nextSibling) {
...
nhn.husky.SE2M_TableEditor = jindo
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				this.oApp.registerLazyMessage([ "EVENT_EDITING_AREA_MOUSEMOVE",
						"STYLE_TABLE" ], [ "hp_SE2M_TableEditor$Lazy.js",
						"SE2M_TableTemplate.js" ])
			}
		});
nhn.husky.SE2M_QuickEditor_Common = jindo
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				...
				this.oApp.registerLazyMessage([ "OPEN_QE_LAYER" ],
						[ "hp_SE2M_QuickEditor_Common$Lazy.js" ])
			},
			$ON_EVENT_EDITING_AREA_KEYDOWN : function(a) {
				var d = a.key();
...
nhn.husky.SE_UndoRedo = jindo
		.$Class({
			...
			$init : function() {
				...
				this.nHardLimit = this.nMaxUndoCount
						+ this.nAfterMaxDeleteBuffer
			},
			...
			$BEFORE_MSG_APP_READY : function() {
				...
				this.oApp.exec("ADD_APP_PROPERTY", [ "getUndoHistory",
						jindo.$Fn(this._getUndoHistory, this).bind() ]);
				this.oApp.exec("ADD_APP_PROPERTY", [ "getUndoStateIdx",
						jindo.$Fn(this._getUndoStateIdx, this).bind() ]);
				this.oApp.exec("ADD_APP_PROPERTY", [ "saveSnapShot",
						jindo.$Fn(this._saveSnapShot, this).bind() ]);
				this.oApp.exec("ADD_APP_PROPERTY", [ "getLastKey",
						jindo.$Fn(this._getLastKey, this).bind() ]);
				this.oApp.exec("ADD_APP_PROPERTY", [ "setLastKey",
						jindo.$Fn(this._setLastKey, this).bind() ]);
				this._saveSnapShot();
				this.oApp.exec("DO_RECORD_UNDO_HISTORY_AT",
						[ this.oCurStateIdx, "", "", "", null,
								this.sDefaultXPath ])
			},
			...
			$ON_MSG_APP_READY : function() {
				...
				this.bFF = a.firefox;
				this.oApp
						.exec("REGISTER_UI_EVENT", [ "undo", "click", "UNDO" ]);
				this.oApp
						.exec("REGISTER_UI_EVENT", [ "redo", "click", "REDO" ]);
			...
			$ON_UNDO : function() {
				...
				this.oApp.exec("RESTORE_UNDO_HISTORY", [
						this.oCurStateIdx.nIdx, this.oCurStateIdx.nStep, b ]);
				this.oApp.exec("CHECK_STYLE_CHANGE", []);
				this.sLastKey = null
			},
			$ON_REDO : function() {
				...
				if (this.oCurStateIdx.nIdx == this.aUndoHistory.length - 1
				...
				this.oApp.exec("RESTORE_UNDO_HISTORY", [
						this.oCurStateIdx.nIdx, this.oCurStateIdx.nStep ]);
				this.oApp.exec("CHECK_STYLE_CHANGE", []);
				this.sLastKey = null
			},
			...
			$ON_RESTORE_UNDO_HISTORY : function(b, m, c) {
				...
				var l = this.aUndoHistory[this.oCurStateIdx.nIdx], h = l.sContent[this.oCurStateIdx.nStep], f = l.sFullContents[this.oCurStateIdx.nStep], g = l.oBookmark[this.oCurStateIdx.nStep], n = l.sParentXPath[this.oCurStateIdx.nStep], a = null, k = "", d = this.oApp
						.getEmptySelection();
				this.oApp.exec("RESTORE_IE_SELECTION");
				if (c) {
			...
			_doRecordUndoHistory : function(r, j) {
				j = j || {};
				var m = j.nStep || 0, s = j.bSkipIfEqual || false, t = j.bTwoStepAction || false, w = j.sSaveTarget
						|| null, a = j.elSaveTarget || null, e = j.bDontSaveSelection || false, k = j.bMustBlockElement || false, n = j.bMustBlockContainer || false, b = j.oUndoCallback, v = j.oRedoCallback;
				this._historyLength = this.aUndoHistory.length;
				if (this.oCurStateIdx.nIdx !== this._historyLength - 1) {
					s = true
				}
				var p = this.aUndoHistory[this.oCurStateIdx.nIdx], h = p.sFullContents[this.oCurStateIdx.nStep], o = "", u = "", d = "", q = null, c = null, f = {
				...
				f.nStep = m;
				if (f.nStep === 0
						&& this.oCurStateIdx.nStep === p.nTotalSteps - 1) {
			...
			_doRecordUndoHistoryAt : function(a, h, g, c, e, j, f, b) {
				if (a.nStep !== 0) {
					...
					d.oRedoCallback = [];
					d.oRedoCallback[0] = b;
					this.aUndoHistory.splice(a.nIdx, this._historyLength
							- a.nIdx, d);
					this._historyLength = this.aUndoHistory.length
				}
			...
			_getTargetXPath : function(c, b) {
				var h = this.sDefaultXPath, f = c[0].sXPath.split("/"), d = c[1].sXPath
						.split("/"), g = [], e = f.length < d.length ? f.length
						: d.length, j = 0, a = -1;
				if (b === "BODY") {
					return h
				}
			...
			_getParentXPath : function(d, e, p) {
				var n = this.sDefaultXPath, k, g, l, f, c, j, h = [ "",
						"HTML[0]", "BODY[0]" ], o = 0, m = -1, b, a;
				if (!d) {
					return n
				}
...
nhn.husky.DialogLayerManager = jindo.$Class({
	...
	$ON_MSG_APP_READY : function() {
		this.oApp.registerLazyMessage([ "SHOW_DIALOG_LAYER",
				"TOGGLE_DIALOG_LAYER" ], [ "hp_DialogLayerManager$Lazy.js",
				"N_DraggableLayer.js" ])
	}
});
...
(function() {
	var d = navigator.userAgent, a = jindo.$Agent(), c = a.navigator(), e = a
			.os();
...
nhn.husky.SE2M_Utils = {
	...
	replaceSpecialChar : function(a) {
		return (typeof (a) == "string") ? (a.replace(/\&/g, "&amp;").replace(
...
(function() {
	...
	IsInstalledFont = function(g) {
		...
		a.style.cssText = h + 'font-family:"' + g.replace(b, '","') + '", "'
				+ c + '" !important';
...
nhn.husky.SE2B_CSSLoader = jindo.$Class({
	...
	aInstantLoadTrigger : [ "OPEN_QE_LAYER", "SHOW_ACTIVE_LAYER",
			"SHOW_DIALOG_LAYER", "START_SPELLCHECK" ],
...
nhn.husky.HuskyCore
		.mixin(
				nhn.husky.SE2M_Quote,
				{
					$ON_TOGGLE_BLOCKQUOTE_LAYER : function() {
						this.oApp.exec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [
								this.elDropdownLayer, null, "SELECT_UI",
								[ "quote" ], "DESELECT_UI", [ "quote" ] ]);
					...
					_unwrapBlock : function(a) {
						...
						if (e.nodeType !== 3) {
							var d = c.getTextNodes() || "", b = d.length - 1;
							e = (b > -1) ? d[b] : null
						}
						while (f.firstChild) {
					...
					_wrapBlock : function(w, f) {
						...
						if (b.startContainer === b.endContainer
							...
							if (nhn.husky.SE2M_Utils
									.isBlankNode(b.startContainer)
									|| nhn.husky.SE2M_Utils.isFirstChildOfNode(
											"IMG", b.startContainer.tagName,
											b.startContainer)
									|| nhn.husky.SE2M_Utils.isFirstChildOfNode(
											"IFRAME", b.startContainer.tagName,
											b.startContainer)) {
						...
						if (this._isExceedMaxDepth(l)) {
							alert(this.oApp
									.$MSG("SE2M_Quote.exceedMaxCount")
									.replace("#MaxCount#", (this.nMaxLevel + 1)));
							this.oApp.exec("HIDE_ACTIVE_LAYER", []);
							p = l.nextSibling;
							m = l.parentNode;
							v = l.childNodes;
				...
				nhn.husky.SE2M_SCharacter,
				{
					_assignHTMLObjects : function(b) {
						...
						this.oTextField = jindo.$$.getSingle("INPUT",
								this.elDropdownLayer);
						this.oInsertButton = jindo.$$.getSingle(
								"BUTTON.se2_confirm", this.elDropdownLayer);
						this.aCloseButton = jindo.$$(
								"BUTTON.husky_se2m_sCharacter_close",
								this.elDropdownLayer);
						this.aSCharList = jindo.$$(
								"UL.husky_se2m_sCharacter_list",
								this.elDropdownLayer);
					...
					$LOCAL_BEFORE_FIRST : function(c) {
						...
						var b = jindo.$Fn(this.oApp.exec, this.oApp)
								.bind("INSERT_SCHARACTERS",
						...
						for (var a = 0; a < this.aLabel.length; a++) {
							...
							jindo.$Fn(d, this).attach(
									this.aLabel[a].firstChild, "mousedown")
						}
						for (var a = 0; a < this.aCloseButton.length; a++) {
							this.oApp.registerBrowserEvent(
									this.aCloseButton[a], "click",
									"HIDE_ACTIVE_LAYER", [])
						}
						this.oApp.registerBrowserEvent(this.elDropdownLayer,
								"click", "EVENT_SCHARACTER_CLICKED", []);
						this.oApp.registerBrowserEvent(this.oTextField,
								"keydown", "EVENT_SCHARACTER_KEYDOWN")
					},
					...
					$ON_CHANGE_SCHARACTER_SET : function(b) {
						for (var a = 0; a < this.aSCharList.length; a++) {
							if (jindo.$Element(this.aLabel[a]).hasClass(
									"active")) {
								if (a == b) {
									return
								}
								jindo.$Element(this.aLabel[a]).removeClass(
										"active")
							}
						}
		...
		.mixin(
				nhn.husky.SE2M_TableCreator,
				{
					_assignHTMLObjects : function(b) {
						...
						this.welDropdownLayer = jindo
								.$Element(this.elDropdownLayer);
						...
						this.elLayer_Dim1 = jindo.$$.getSingle(
								"DIV.se2_t_dim0", this.elDropdownLayer);
						this.elLayer_Dim2 = jindo.$$.getSingle(
								"DIV.se2_t_dim3", this.elDropdownLayer);
						a = jindo.$$("SPAN.se2_pre_color>BUTTON",
								this.elDropdownLayer);
						this.elBtn_borderColor = a[0];
						this.elBtn_BGColor = a[1];
						this.elBtn_tableStyle = jindo.$$.getSingle(
								"DIV.se2_select_ty2>BUTTON",
								this.elDropdownLayer);
						a = jindo.$$("P.se2_btn_area>BUTTON",
								this.elDropdownLayer);
						...
						this.sNoBorderText = jindo.$$.getSingle(
								"SPAN.se2m_no_border", this.elDropdownLayer).innerHTML;
					...
					$LOCAL_BEFORE_FIRST : function() {
						...
						this.oApp.registerBrowserEvent(this.elText_row,
								"change", "TABLE_SET_ROW_NUM", [ null, 0 ]);
						this.oApp.registerBrowserEvent(this.elText_col,
								"change", "TABLE_SET_COLUMN_NUM", [ null, 0 ]);
						this.oApp.registerBrowserEvent(this.elText_borderSize,
								"change", "TABLE_SET_BORDER_SIZE", [ null, 0 ]);
						this.oApp.registerBrowserEvent(this.elBtn_rowInc,
								"click", "TABLE_INC_ROW");
						this.oApp.registerBrowserEvent(this.elBtn_rowDec,
								"click", "TABLE_DEC_ROW");
						jindo.$Fn(this._numRowKeydown, this).attach(
								this.elText_row.parentNode, "keydown");
						this.oApp.registerBrowserEvent(this.elBtn_colInc,
								"click", "TABLE_INC_COLUMN");
						this.oApp.registerBrowserEvent(this.elBtn_colDec,
								"click", "TABLE_DEC_COLUMN");
						jindo.$Fn(this._numColKeydown, this).attach(
								this.elText_col.parentNode, "keydown");
						this.oApp.registerBrowserEvent(
								this.elBtn_incBorderSize, "click",
								"TABLE_INC_BORDER_SIZE");
						this.oApp.registerBrowserEvent(
								this.elBtn_decBorderSize, "click",
								"TABLE_DEC_BORDER_SIZE");
						jindo.$Fn(this._borderSizeKeydown, this).attach(
								this.elText_borderSize.parentNode, "keydown");
						this.oApp.registerBrowserEvent(this.elBtn_borderStyle,
								"click", "TABLE_TOGGLE_BORDER_STYLE_LAYER");
						this.oApp.registerBrowserEvent(this.elBtn_tableStyle,
								"click", "TABLE_TOGGLE_STYLE_LAYER");
						this.oApp.registerBrowserEvent(this.elBtn_borderColor,
								"click", "TABLE_TOGGLE_BORDER_COLOR_PALLET");
						this.oApp.registerBrowserEvent(this.elBtn_BGColor,
								"click", "TABLE_TOGGLE_BGCOLOR_PALLET");
						this.oApp.registerBrowserEvent(
								this.elRadio_manualStyle, "click",
								"TABLE_ENABLE_MANUAL_STYLE");
						this.oApp.registerBrowserEvent(
								this.elRadio_templateStyle, "click",
								"TABLE_ENABLE_TEMPLATE_STYLE");
						...
						for (a = 0; a < this.aElBtn_borderStyle.length; a++) {
							this.oApp.registerBrowserEvent(
									this.aElBtn_borderStyle[a], "click",
									"TABLE_SELECT_BORDER_STYLE")
						}
						for (a = 0; a < this.aElBtn_tableStyle.length; a++) {
							this.oApp.registerBrowserEvent(
									this.aElBtn_tableStyle[a], "click",
									"TABLE_SELECT_STYLE")
						}
						this.oApp.registerBrowserEvent(this.elBtn_apply,
								"click", "TABLE_INSERT");
						this.oApp.registerBrowserEvent(this.elBtn_cancel,
						...
						this.oApp.registerBrowserEvent(this.elDropdownLayer,
								"keydown", "EVENT_TABLE_CREATE_KEYDOWN");
					...
					_getTableString : function() {
						...
						if (this.nStyleMode == 1) {
							a = this._doGetTableString(this.nColumns,
									this.nRows, this.nBorderSize,
									this.sBorderColor, this.sBGColor,
									this.nBorderStyleIdx)
						} else {
							a = this._doGetTableString(this.nColumns,
									this.nRows, this.nBorderSize,
									this.sBorderColor, this.sBGColor, 0)
						}
						return a
					},
					$ON_TABLE_INSERT : function() {
						...
						if (a.firefox) {
							h = this.oApp.getWYSIWYGDocument().createElement(
									"BR");
							e.parentNode.insertBefore(h, e.nextSibling)
						} else {
							if (a.ie) {
								h = this.oApp.getWYSIWYGDocument()
										.createElement("p");
								e.parentNode.insertBefore(h, e.nextSibling)
							}
						}
						if (this.nStyleMode == 2) {
							this.oApp.exec("STYLE_TABLE", [ e,
									this.nTableStyleIdx ])
						}
						...
						this.oApp.exec("HIDE_ACTIVE_LAYER", []);
						this.oApp.exec("MSG_DISPLAY_REEDIT_MESSAGE_SHOW", [
								this.name, this.sReEditGuideMsg_table ])
					},
					_divideParagraph : function(f) {
						var a, d, o, l, n, h;
						f.fixCommonAncestorContainer();
						var j = f.commonAncestorContainer;
						var k = nhn.husky.SE2M_Utils
								.findAncestorByTagNameWithCount("P", j);
						var m = k.elNode;
						if (m) {
							var c = nhn.husky.SE2M_Utils
									.findClosestAncestorAmongTagNamesWithCount(
											[ "TD", "TH" ], j);
							var e = c.elNode;
							if (e) {
						...
						if (!a.firstChild
								|| nhn.husky.SE2M_Utils.isBlankNode(a)) {
							f.selectNode(a);
							f.select();
							return f
						}
					...
					$ON_TABLE_SET_BORDER_SIZE : function(b, a) {
						b = b || parseInt(this.elText_borderSize.value, 10)
								|| 0;
						a = a || 0;
						b += a;
					...
					$ON_TABLE_TOGGLE_BORDER_COLOR_PALLET : function() {
						if (this.welDropdownLayer.hasClass("p1")) {
							this.oApp
									.exec("TABLE_HIDE_BORDER_COLOR_PALLET", [])
						} else {
							this.oApp
									.exec("TABLE_SHOW_BORDER_COLOR_PALLET", [])
						}
					},
					...
					_selectBorderStyle : function(a) {
						this.elPanel_borderStylePreview.className = "se2_b_style"
								+ a;
						this.nBorderStyleIdx = a;
						this._drawTableDropdownLayer()
					},
					_selectTableStyle : function(a) {
						this.elPanel_tableStylePreview.className = "se2_t_style"
								+ a;
					...
					_loadValuesFromHTML : function() {
						...
						this.nBorderSize = parseInt(
								this.elText_borderSize.value, 10) || 1;
					...
					_doGetTableString : function(d, k, h, j, n, l) {
						...
						var c = this.aTableStyleByBorder[l].replace(
								/#BorderSize#/g, this.nBorderSize).replace(
								/#BorderSize\*([0-9]+)#/g, function(p, o) {
									return h * parseInt(o, 10)
								}).replace(/#BorderSize\+([0-9]+)#/g,
								function(p, o) {
									return h + parseInt(o, 10)
								}).replace("#BorderColor#", this.sBorderColor)
								.replace("#BGColor#", this.sBGColor);
						var g = this.aTDStyleByBorder[l].replace(
								/#BorderSize#/g, this.nBorderSize).replace(
								/#BorderSize\*([0-9]+)#/g, function(p, o) {
									return h * parseInt(o, 10)
								}).replace(/#BorderSize\+([0-9]+)#/g,
		...
		.mixin(
				nhn.husky.SE2M_TableEditor,
				{
					...
					_assignHTMLObjects : function() {
						...
						this.elBtnAddRowBelow = jindo.$$.getSingle(
								"BUTTON.se2_addrow", this.elQELayer);
						this.elBtnAddColumnRight = jindo.$$.getSingle(
								"BUTTON.se2_addcol", this.elQELayer);
						this.elBtnSplitRow = jindo.$$.getSingle(
								"BUTTON.se2_seprow", this.elQELayer);
						this.elBtnSplitColumn = jindo.$$.getSingle(
								"BUTTON.se2_sepcol", this.elQELayer);
						this.elBtnDeleteRow = jindo.$$.getSingle(
								"BUTTON.se2_delrow", this.elQELayer);
						this.elBtnDeleteColumn = jindo.$$.getSingle(
								"BUTTON.se2_delcol", this.elQELayer);
						this.elBtnMergeCell = jindo.$$.getSingle(
								"BUTTON.se2_merrow", this.elQELayer);
						...
						this.elPanelDimDelRow = jindo.$$.getSingle(
								"DIV.husky_se2m_tbl_qe_dim_del_row",
								this.elQELayer);
						...
						for (i = 0; i < this.aElBtn_tableStyle.length; i++) {
							this.oApp.registerBrowserEvent(
									this.aElBtn_tableStyle[i], "click",
									"TABLE_QE_SELECT_TEMPLATE")
						}
					},
					$LOCAL_BEFORE_FIRST : function(a) {
						...
						this.nPageLeftRightMargin = parseInt(b
								.css("marginLeft"), 10)
								+ parseInt(b.css("marginRight"), 10);
						this.nPageTopBottomMargin = parseInt(
								b.css("marginTop"), 10)
						...
						this.oApp.registerBrowserEvent(this.elBtnMergeCell,
								"click", "TE_MERGE_CELLS");
						this.oApp.registerBrowserEvent(this.elBtnSplitColumn,
								"click", "TE_SPLIT_COLUMN");
						this.oApp.registerBrowserEvent(this.elBtnSplitRow,
								"click", "TE_SPLIT_ROW");
						this.oApp.registerBrowserEvent(
								this.elBtnAddColumnRight, "click",
								"TE_INSERT_COLUMN_RIGHT");
						this.oApp.registerBrowserEvent(this.elBtnAddRowBelow,
								"click", "TE_INSERT_ROW_BELOW");
						this.oApp.registerBrowserEvent(this.elBtnDeleteColumn,
								"click", "TE_DELETE_COLUMN");
						this.oApp.registerBrowserEvent(this.elBtnDeleteRow,
								"click", "TE_DELETE_ROW");
						this.oApp.registerBrowserEvent(
								this.elInputRadioBGColor, "click",
								"DRAW_QE_RADIO_OPTION", [ 2 ]);
						this.oApp.registerBrowserEvent(this.elInputRadioBGImg,
								"click", "DRAW_QE_RADIO_OPTION", [ 3 ]);
						this.oApp.registerBrowserEvent(
								this.elInputRadioTemplate, "click",
								"DRAW_QE_RADIO_OPTION", [ 4 ]);
						this.oApp.registerBrowserEvent(this.elBtnBGPalette,
								"click", "TABLE_QE_TOGGLE_BGC_PALETTE");
						this.oApp.registerBrowserEvent(this.elBtnBGIMGPalette,
								"click", "TABLE_QE_TOGGLE_IMG_PALETTE");
						this.oApp.registerBrowserEvent(
								this.elPanelBGIMGPaletteHolder, "click",
								"TABLE_QE_SET_IMG_FROM_PALETTE");
						this.oApp.registerBrowserEvent(this.elBtnQETemplate,
								"click", "TABLE_QE_TOGGLE_TEMPLATE");
						this.oApp.registerBrowserEvent(document.body,
								"mouseup", "EVENT_OUTER_DOC_MOUSEUP");
						this.oApp.registerBrowserEvent(document.body,
								"mousemove", "EVENT_OUTER_DOC_MOUSEMOVE");
						this._rxCellNames = new RegExp("^("
								+ this._aCellName.join("|") + ")$", "i")
					},
					$ON_EVENT_EDITING_AREA_KEYUP : function(a) {
						var b = a.key();
						if (b.keyCode == 229 || b.alt || b.ctrl
								|| b.keyCode == 16) {
							return
						} else {
					...
					$ON_TABLE_QE_SET_BGC : function(c) {
						...
						for (var a = 0, b = this.aSelectedCells.length; a < b; a++) {
							this.aSelectedCells[a].setAttribute(
									this.TMP_BGC_ATTR, c);
							this.aSelectedCells[a]
									.removeAttribute(this.TMP_BGIMG_ATTR)
						}
					...
					$ON_SHOW_COMMON_QE : function() {
						if (jindo.$Element(this.elSelectionStartTable)
								.hasClass(this._sSETblClass)) {
							this.oApp.exec("SHOW_TABLE_QE")
						} else {
							if (jindo.$Element(this.elSelectionStartTable)
									.hasClass(this._sSEReviewTblClass)) {
					...
					$ON_SETUP_TABLE_QE_MODE : function(d) {
						...
						this.oApp.exec("TABLE_QE_DIM", [ this.QE_DIM_MERGE_BTN,
								b ]);
						var c = this.aSelectedCells[0]
								.getAttribute(this.TMP_BGC_ATTR)
								|| "rgb(255,255,255)";
						var q = true;
						...
						if (this.nQEMode === 0) {
							...
							jindo.$Element(this.elPanelTableBGArea).className(
									"se2_qe2");
							...
							this.elPanelQETemplatePreview.className = "se2_t_style"
									+ m;
							...
							if (this.nQEMode == 1) {
								...
								var m = this
										.parseIntOr0(this.elSelectionStartTable
					...
					$ON_TE_MERGE_CELLS : function() {
						...
						var f = nhn.husky.SE2M_Utils.findAncestorByTagName(
								"TABLE", g);
						var h, e;
						var b, c = this.aSelectedCells[0];
						h = parseInt(
								c.style.height || c.getAttribute("height"), 10);
						e = parseInt(c.style.width || c.getAttribute("width"),
								10);
						...
						g.setAttribute("colSpan", this.htSelectionEPos.x
								- this.htSelectionSPos.x + 1);
						g.setAttribute("rowSpan", this.htSelectionEPos.y
						...
						for (d = 1; d < this.aSelectedCells.length; d++) {
							...
							if (j.parentNode) {
								...
								var a = jindo.$Agent().navigator();
								if (a.ie
										&& (a.nativeVersion == 9 || a.nativeVersion == 10)
										&& (a.version == 9 || a.version == 10)) {
					...
					$ON_TE_SPLIT_COLUMN : function() {
						...
						for (var c = 0, l = this.aSelectedCells.length; c < l; c++) {
							...
							var a = jindo.$Agent().navigator();
							if (a.ie
									&& (a.nativeVersion >= 9 || a.nativeVersion <= 11)
									&& (a.version >= 9 || a.version <= 11)) {
					...
					$ON_TE_SPLIT_ROW : function() {
						...
						var n = jindo.$$(">TBODY>TR",
								this.elSelectionStartTable, {
									oneTimeOffCache : true
								});
						this._removeClassFromSelection();
						var w = 0;
						...
						for (var s = 0, l = this.aSelectedCells.length; s < l; s++) {
							...
							c = n[a.y];
							o = this.oApp.getWYSIWYGDocument().createElement(
									"TR");
						...
						for (var s = 0, l = this.aSelectedCells.length; s < l; s++) {
							...
							for (var m = 0, g = k.length; m < g; m++) {
								y = k[m];
								if (!y.tagName
										|| !this._rxCellNames.test(y.tagName)) {
									continue
								}
							...
							if (j.ie
									&& (j.nativeVersion >= 9 || j.nativeVersion <= 11)
									&& (j.version >= 9 || j.version <= 11)) {
					...
					$ON_EVENT_EDITING_AREA_MOUSEDOWN : function(c) {
						...
						case this.STATUS.S_0:
							...
							var b = nhn.husky.SE2M_Utils
									.findClosestAncestorAmongTagNames(
											this._aCellName, c.element);
							if (b && this._rxCellNames.test(b.tagName)) {
								var a = nhn.husky.SE2M_Utils
										.findAncestorByTagName("TABLE", b);
								if (!jindo.$Element(a).hasClass(
										this._sSETblClass)
					...
					$ON_EVENT_EDITING_AREA_MOUSEMOVE : function(c) {
						...
						case this.STATUS.MOUSEDOWN_CELL:
							...
							if ((b && b !== this.elSelectionStartTD) || !b) {
								...
								this
										._reassignCellSizes(this.elSelectionStartTable);
								this._startCellSelection();
								this._selectBetweenCells(
										this.elSelectionStartTD, b)
							}
							break;
						case this.STATUS.CELL_SELECTING:
						...
						case this.STATUS.CELL_SELECTING:
							...
							if (b < this.htEditingAreaPos.top) {
								...
								if (f > 0) {
									...
									var c = this.oApp.getSelection();
									c
											.selectNodeContents(this.aSelectedCells[0]);
									c.select();
					...
					_eventEditingAreaMouseup : function(a) {
						...
						case this.STATUS.CELL_SELECTING:
							this
									._changeTableEditorStatus(this.STATUS.CELL_SELECTED);
							break;
						case this.STATUS.CELL_SELECTED:
							break
						}
						...
						for (var p = 0; p < 2; p++) {
							for (var s = 0; s < d[p].length; s++) {
								...
								if (g.getComputedStyle) {
									x = parseFloat(
											getComputedStyle(g).paddingLeft, 10);
									r = parseFloat(
											getComputedStyle(g).paddingRight,
											10);
									y = parseFloat(
											getComputedStyle(g).paddingTop, 10);
									v = parseFloat(
											getComputedStyle(g).paddingBottom,
											10);
									o = parseFloat(
											getComputedStyle(g).borderLeftWidth,
											10);
									f = parseFloat(
											getComputedStyle(g).borderRightWidth,
											10);
									e = parseFloat(
											getComputedStyle(g).borderTopWidth,
											10);
									z = parseFloat(
									...
									o = this.parseBorder(b
											.css("borderLeftWidth"), b
											.css("borderLeftStyle"));
									f = this.parseBorder(b
											.css("borderRightWidth"), b
											.css("borderRightStyle"));
									e = this.parseBorder(b
											.css("borderTopWidth"), b
											.css("borderTopStyle"));
									z = this.parseBorder(b
											.css("borderBottomWidth"), b
					...
					_mousedown_ResizeCover : function(c) {
						...
						this.wfnMousemove_ResizeCover.attach(
								this.elResizeCover, "mousemove");
						...
						this.nClientXDiff = this.nStartWidth
								- this.htResizing.htEPos.clientX;
						this.nClientYDiff = this.nStartHeight
								- this.htResizing.htEPos.clientY;
						this._reassignCellSizes(this.htResizing.elTable);
						this.htMap = this
								._getCellMapping(this.htResizing.elTable);
						var d = this
								._getBasisCellPosition(this.htResizing.elCell);
						...
						this.htAllAffectedCells = this._getAllAffectedCells(a,
								e, this.htResizing.nResizeMode,
								this.htResizing.elTable)
					},
					_mousemove_ResizeCover : function(b) {
						if (jindo.$Agent().navigator().chrome
								|| jindo.$Agent().navigator().safari) {
							if (this.htResizing.nPreviousResizeMode != undefined
									&& this.htResizing.nPreviousResizeMode != 0) {
						...
						if (this.htResizing.nResizeMode == 1) {
							this.elResizeGrid.style.left = b.pos().clientX
									- this.nClientXDiff
									- this
											.parseIntOr0(this.elResizeGrid.style.width)
									/ 2 + "px"
						} else {
							this.elResizeGrid.style.top = b.pos().clientY
									- this.nClientYDiff
									- this
					...
					_mouseup_ResizeCover : function(h) {
						...
						this.wfnMousemove_ResizeCover.detach(
								this.elResizeCover, "mousemove");
						...
						if (this.htResizing.nResizeMode == 1) {
							...
							if (this.htAllAffectedCells.nMinBefore != -1
									&& e < -1
											* this.htAllAffectedCells.nMinBefore) {
						...
						for (var d = 0; d < c.length; d++) {
							...
							j += f;
							g.style.width = Math.max(a, this.MIN_CELL_WIDTH)
									+ "px";
							g.style.height = Math.max(j, this.MIN_CELL_HEIGHT)
									+ "px"
						}
					...
					_changeTableEditorStatus : function(a) {
						...
						this.oApp.exec("TABLE_EDITOR_STATUS_CHANGED",
								[ this.nStatus ])
					},
					_isOnBorder : function(e) {
						...
						if (jindo.$Agent().navigator().ie
								|| jindo.$Agent().navigator().safari) {
							d = this.htResizing.nBorderSize
									+ this.nDraggableCellEdge;
							b = this.nDraggableCellEdge
						} else {
							d = this.nDraggableCellEdge;
							b = this.htResizing.nBorderSize
									+ this.nDraggableCellEdge
						}
						var c = this.htResizing.elCell.clientWidth, a = this.htResizing.elCell.clientHeight;
						nRightBorderCriteria = c
								- this.htResizing.htEPos.offsetX,
								nBottomBorderCriteria = a
					...
					_showCellResizeGrip : function() {
						...
						if (this.htResizing.nResizeMode == 1) {
							...
							this.elResizeGrid.style.left = this.htResizing.elCell.clientWidth
									+ this.htResizing.htEPos.clientX
									- this.htResizing.htEPos.offsetX
									- this
											.parseIntOr0(this.elResizeGrid.style.width)
							...
							if (this.htResizing.nResizeMode == 2) {
								var a = this.oApp.elEditingAreaContainer.offsetWidth
										+ "px";
								this
										._setResizerSize(
												this.parseIntOr0(a),
												(this.htResizing.nBorderSize + this.nDraggableCellEdge) * 2);
								this.elResizeGrid.style.top = this.htResizing.elCell.clientHeight
										+ this.htResizing.htEPos.clientY
										- this.htResizing.htEPos.offsetY
										- this
												.parseIntOr0(this.elResizeGrid.style.height)
													...
													this
															.parseIntOr0(jindo
																	...
																	.css(
																			"height")));
									this.elResizeGrid.style.top = "0px";
									this.elResizeGrid.style.left = +this.htResizing.htEPos.clientX
											- this.htResizing.htEPos.offsetX
											- this
													.parseIntOr0(this.elResizeGrid.style.width)
											/ 2 + "px";
									...
									if (this.htResizing.nResizeMode == 4) {
										...
										this
												._setResizerSize(
														this.parseIntOr0(a),
														(this.htResizing.nBorderSize + this.nDraggableCellEdge) * 2);
										this.elResizeGrid.style.top = this.htResizing.htEPos.clientY
												- this.htResizing.htEPos.offsetY
												- this
														.parseIntOr0(this.elResizeGrid.style.height)
												/ 2 + "px";
					...
					_createCellResizeGrip : function() {
						...
						var a = jindo.$$
								.getSingle(".husky_seditor_editing_area_container");
					...
					_deleteSelectedCells : function() {
						...
						for (var b = 0, f = this.aSelectedCells.length; b < f; b++) {
							...
							if (e.ie
									&& (e.nativeVersion == 9 || e.nativeVersion == 10)
									&& (e.version == 9 || e.version == 10)) {
								this._removeEmptyTextNode_IE(g)
							}
							g.parentNode.removeChild(g)
						}
						var d = jindo.$$(">TBODY>TR",
								this.elSelectionStartTable, {
									oneTimeOffCache : true
								});
						var c = this.htSelectionEPos.x - this.htSelectionSPos.x
								+ 1;
					...
					_insertColumnAfter : function() {
						...
						for (var l = 0, o = this.htMap[0].length; l < o; l++) {
							...
							for (var m = this.htSelectionEPos.x; m >= this.htSelectionSPos.x; m--) {
								...
								k.insertBefore(b, g);
								if (c.ie
										&& (c.nativeVersion >= 9 || c.nativeVersion <= 11)
						...
						var n = this.htSelectionEPos.x - this.htSelectionSPos.x
								+ 1;
						var d = this.htSelectionEPos.y - this.htSelectionSPos.y
								+ 1;
					...
					_insertRowBelow : function() {
						...
						for (var o = this.htSelectionSPos.y; o <= this.htSelectionEPos.y; o++) {
							...
							if (c.ie
								...
								for (var f = 0, k = q.length; f < k; f++) {
									var b = q[f];
									var a = e[f];
									if (this._rxCellNames.test(b.nodeName)
											&& a
											&& this._rxCellNames
													.test(a.nodeName)) {
						...
						var p = this.htSelectionEPos.x - this.htSelectionSPos.x
								+ 1;
						var d = this.htSelectionEPos.y - this.htSelectionSPos.y
								+ 1;
					...
					_startCellSelection : function() {
						...
						this
								._changeTableEditorStatus(this.STATUS.CELL_SELECTING)
					},
					...
					_getCellMapping : function(u) {
						...
						for (var t = 0; t < o.length; t++) {
							...
							if (p.getAttribute("colSpan")) {
								w += this
										.parseIntOr0(p.getAttribute("colSpan"))
							} else {
								w++
							}
						}
						...
						for (var h = 0, s = 0, d = r[0].length; h < d; h++, s++) {
							...
							if (!n[h].innerHTML.match(/TD|TH/i)) {
								for (var j = 0, a = r.length; j < a; j++) {
									...
									v = e;
									var c = parseInt(e.getAttribute("rowSpan"),
											10) || 1;
									if (c > 1) {
								...
								if (k.ie
										&& (k.nativeVersion == 9 || k.nativeVersion == 10)
										&& (k.version == 9 || k.version == 10)) {
					...
					_applyTableTemplate : function(b, a) {
						...
						this._doApplyTableTemplate(b,
								nhn.husky.SE2M_TableTemplate[a], false);
					...
					_hideTableTemplate : function(a) {
						if (a.getAttribute(this.ATTR_TBL_TEMPLATE)) {
							this
									._doApplyTableTemplate(
											a,
											nhn.husky.SE2M_TableTemplate[this
													.parseIntOr0(a
															.getAttribute(this.ATTR_TBL_TEMPLATE))],
											true)
						}
		...
		.mixin(
				nhn.husky.SE2M_Hyperlink,
				{
					...
					_validateTarget : function() {
						var b = jindo.$Agent().navigator(), a = true;
						if (b.ie) {
							jindo
					...
					_isExceptional : function() {
						...
						if (this.oApp.getWYSIWYGDocument().selection
								&& this.oApp.getWYSIWYGDocument().selection.type === "None") {
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_LineHeightWithLayerUI, {
	...
	$ON_SE2M_INC_LINEHEIGHT : function() {
		this.oInput.value = parseInt(this.oInput.value, 10)
		...
		.mixin(
				nhn.husky.SE2M_QuickEditor_Common,
				{
					setOpenType : function(b, a) {
						if (typeof (this._environmentData) == "undefined"
								|| this._environmentData == null) {
							this._environmentData = {}
						}
						if (typeof (this._environmentData[b]) == "undefined"
								|| this._environmentData[b] == null) {
							this._environmentData[b] = {}
						}
						if (typeof (this._environmentData[b].isOpen) == "undefined"
								|| this._environmentData[b].isOpen == null) {
							this._environmentData[b].isOpen = true
						}
						this._environmentData[b].isOpen = a
					},
					$ON_OPEN_QE_LAYER : function(b, a, g) {
						if (this.waHotkeys.length() > 0
								&& !this.waHotkeyLayers.has(a)) {
							this.waHotkeyLayers.push(a);
							var f;
							for (var c = 0, e = this.waHotkeys.length(); c < e; c++) {
								f = this.waHotkeys.get(c);
								this.oApp.exec("ADD_HOTKEY", [ f[0], f[1],
										f[2], a ])
							}
						}
						var d = g;
					...
					$LOCAL_BEFORE_FIRST : function(b) {
						...
						new jindo.DragArea(this._qe_wrap, {
							...
							dragStart : function(e) {
								if (!jindo.$Element(e.elDrag).hasClass(
										"se2_qmax")) {
					...
					positionCopy : function(b, a, c) {
						jindo.$Element(
								jindo.$$.getSingle("._" + c, this.currentEle))
								.css({
									top : a,
									left : b
								})
						...
						var b = jindo.$$
								.getSingle(
										"._"
												+ this._environmentData[this._currentType].type,
										this.currentEle);
						jindo.$Element(b).show().css(
								this.get_position_layer(a, b));
						this.lazy_common()
					},
				...
				if (this.document.body.createTextRange) {
					try {
						...
						if (this.document.selection) {
							...
							if (!this._range) {
								this._range = this.document.body
										.createTextRange()
							} else {
								this._range.collapse(false)
							}
